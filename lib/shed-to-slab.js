...
var translators = {
    ...
    typeApplication: function(shedTypeApplication) {
        var slabFunc = translate(shedTypeApplication.func);
        var slabArgs = shedTypeApplication.parameters.map(translate);
        return slab.typeApplication(slabFunc, slabArgs, shedTypeApplication);
    },
    functionType: function(shedFunctionType) {
        var slabFunc = slab.ref("$Function", shedFunctionType);
        var slabArgs = shedFunctionType.argumentTypes.map(translate)
            .concat([translate(shedFunctionType.returnType)]);
        return slab.typeApplication(slabFunc, slabArgs, shedFunctionType);
    },
    ...
    lambda: function(shedLambda) {
        ...
        return translateDeclarationWithFormalTypeParameters(shedLambda, slabLambda);
    },
    class: function(shedClass) {
        var slabClass = slab.class(
            translate(shedClass.formalArguments),
            translateMembers(shedClass),
            shedClass.body.map(translate),
            shedClass
        );
        return translateDeclarationWithFormalTypeParameters(shedClass, slabClass);
    },
    object: function(shedObject) {
        var classIdentifier = "$classForObject";
        var slabBody = shedObject.body.map(translate);
        var slabClass = slab.class(slab.formalArguments([], shedObject), [], slabBody, shedObject);
    ...
    public: function(declaration) {
        return translate(declaration.declaration);
    },
    ...
    module: function(shedModule) {
        ...
        var package = shedModule.packageDeclaration.map(function(declaration) {
...
var translateDeclarationWithFormalTypeParameters = function(shedDeclaration, slabDeclaration) {
    return shedDeclaration.formalTypeParameters.map(function(shedFormalTypeParameters) {
        return slab.lambda(
            translate(shedFormalTypeParameters),
            options.none,
            slabDeclaration,
            shedDeclaration
        );
    }).orElse(slabDeclaration);
};
