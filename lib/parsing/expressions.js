...
var indent = tokenRules.indent;
var dedent = tokenRules.dedent;
var newLine = tokenRules.newLine;
...
var typeExpressionRule = firstOf(
    "type expression",
    typeApplicationRule,
    functionTypeRule,
...
var formalTypeParametersRule = lazyRule(function() {
    var formalTypeParameters = capture(oneOrMoreWithSeparator(formalTypeParameterRule, symbol(",")), "formalTypeParameter");
    return then(
        sequence(symbol("["), sequence.cut(), formalTypeParameters, symbol("]"), symbol("=>")),
...
var classRule = lazyRule(function() {
    ...
    return then(
        ...
        applySequenceValuesToNode(nodes.class, formalTypeParameters, formalArguments, body)
    );
});
...
var indentedExpressionRule = lazyRule(function() {
    var expression = sequence.capture(expressionRule, "expression");
    return then(
        sequence(
            newLine(),
...
var partialMemberAccess = lazyRule(function() {
    var memberName = sequence.capture(identifier(), "memberName");
    return then(
...
var ifExpressionRule = lazyRule(function() {
    ...
    return then(
        sequence(keyword("if"), sequence.cut(), condition, keyword("then"), ifTrue, elseIfs, ifFalse),
...
var newLineAndIndent = function(input) {
    if (input.justIndented) {
        return lop.results.success(null, input, input.to(input));
    } else {
        return lop.results.failure([""], input);
    }
};

var expressionRule = firstOf(
    "expression",
    blockExpressionRule,
    ifExpressionRule,
    assignmentRule,
...
exports.expression = expressionRule;
exports.typeExpression = typeExpressionRule;
exports.typeSpecifier = typeSpecifierRule;
exports.formalArguments = formalArgumentsRule;
exports.formalTypeParameters = formalTypeParametersRule;
exports.definition = definitionRule;
