...
var partialMemberAccess = lazyRule(function() {
    ...
    return sequence(symbol("."), sequence.cut(), memberName)
        .map(function(memberName) {
            return function(left, source) {
                return nodes.memberAccess(left, memberName, source);
            };
        });
});
...
var partialTypeApplicationRule = then(
    typeParameterListRule,
    function(parameters) {
        return function(left, source) {
            return nodes.typeApplication(left, parameters, source);
        };
    }
);
...
var partialAssignRule = lazyRule(function() {
    return sequence(
            symbol("="),
            sequence.cut(),
            sequence.capture(expressionParser.rightAssociative("assign"))
        ).map(function(value) {
            return function(left, source) {
                return nodes.assign(left, value, source);
            };
        });
});
...
var expressionParser = bottomUp.parser("expression",
    ...
    [
        ...
        bottomUp.infix("member", partialMemberAccess),
        bottomUp.infix("typeApply", partialTypeApplicationRule),
        bottomUp.infix("assign", partialAssignRule)
    ]
);
