...
var expressionWriters = {
    ...
    memberAccess: function(jsMemberAccess) {
        return util.format("(%s).%s", write(jsMemberAccess.left), jsMemberAccess.memberName);
    },
    ...
    conditionalOperator: function(jsConditional) {
        return util.format(
            "(%s) ? (%s) : (%s)",
            write(jsConditional.condition),
            write(jsConditional.ifTrue),
            write(jsConditional.ifFalse)
        );
    }
};
